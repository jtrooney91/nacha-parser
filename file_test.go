...
func TestFileADVControlValidate(t *testing.T) {
	...
	err := file.Validate()
	if !Match(err, NewErrFileCalculatedControlEquality("TotalDebitEntryDollarAmountInFile", 0, 0)) {
		t.Errorf("%T: %s", err, err)
	}
}
...
func TestFileControlValidate(t *testing.T) {
	...
	err := file.Validate()
	if !Match(err, NewErrFileCalculatedControlEquality("TotalDebitEntryDollarAmountInFile", 0, 22)) {
		t.Errorf("%T: %s", err, err)
	}
}
