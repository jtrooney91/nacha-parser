...
func testFileBatchHeaderDuplicate(t testing.TB) {
	...
	_, err := r.Read()
	if !Has(err, ErrFileConsecutiveBatchHeaders) {
		t.Errorf("%T: %s", err, err)
	}
}
...
func testFileAddenda05(t testing.TB) {
	...
	r := NewReader(strings.NewReader(line))
	_, err := r.Read()
	if !Has(err, ErrBatchAddendaIndicator) {
		t.Errorf("%T: %s", err, err)
	}
}
...
func testFileBatchControlValidate(t testing.TB) {
	...
	r := NewReader(strings.NewReader(line))
	_, err := r.Read()
	if !Has(err, NewErrBatchHeaderControlEquality(220, 200)) {
		t.Errorf("%T: %s", err, err)
	}
}
...
func testFileAddBatchValidation(t testing.TB) {
	...
	r := NewReader(strings.NewReader(line))
	_, err := r.Read()
	if !Has(err, NewErrBatchCalculatedControlEquality(1, 0)) {
		t.Errorf("%T: %s", err, err)
	}
}
...
func testACHFileRead(t testing.TB) {
	...
	if !Match(err2, NewErrFileCalculatedControlEquality("BatchCount", 2, 5)) {
...
func TestIATAddendaRecordIndicator(t *testing.T) {
	...
	if !Has(err, ErrIATBatchAddendaIndicator) {
		t.Errorf("%T: %s", err, err)
	}
}
...
func TestACHFileIATAddenda10(t *testing.T) {
	...
	if !Has(err, ErrTransactionTypeCode) {
		t.Errorf("%T: %s", err, err)
	}
}
...
func TestACHFileIATAddenda98(t *testing.T) {
	...
	if !Has(err, ErrAddenda98ChangeCode) {
		t.Errorf("%T: %s", err, err)
	}
}
...
func TestACHFileIATAddenda99(t *testing.T) {
	...
	if !Has(err, ErrAddenda99ReturnCode) {
		t.Errorf("%T: %s", err, err)
	}
}
...
func testACHFileIATBC(t testing.TB) {
	...
	if !Has(err, NewErrBatchHeaderControlEquality(23138010, 23100000)) {
		t.Errorf("%T: %s", err, err)
	}
}
...
func TestADVReturnError(t *testing.T) {
	...
	_, err := r.Read()
	if !Has(err, NewErrBatchCalculatedControlEquality(1, 2)) {
		t.Errorf("%T: %s", err, err)
	}
}
...
func TestReader_AddendaParse(t *testing.T) {
	...
	if err := r.parseAddenda(); err != nil {
		if !Match(err, ErrFileAddendaOutsideBatch) {
			t.Errorf("%T: %s", err, err)
		}
	}
}
