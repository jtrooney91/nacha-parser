...
func testBlockingFactor(t testing.TB) {
	...
	err := fh.Validate()
	if !Match(err, ErrBlockingFactor) {
		t.Errorf("%T: %s", err, err)
	}
}
...
func testUpperLengthFileID(t testing.TB) {
	...
	err = fh.Validate()
	if !Match(err, NewErrValidFieldLength(1)) {
		t.Errorf("%T: %s", err, err)
	}
}
...
func testImmediateDestinationNameAlphaNumeric(t testing.TB) {
	...
	err := fh.Validate()
	if !Match(err, ErrNonAlphanumeric) {
		t.Errorf("%T: %s", err, err)
	}
}
...
func TestFHFieldInclusionFileCreationDate(t *testing.T) {
	fh := mockFileHeader()
	fh.FileCreationDate = ""
	if err := fh.Validate(); !Match(err, ErrConstructor) {
		t.Errorf("%T: %s", err, err)
	}
}
...
func testFileHeaderCreationDate(t testing.TB) {
	fh := mockFileHeader()
	fh.FileCreationDate = time.Now().AddDate(0, 0, 1).Format("060102") // YYMMDD
	if err := fh.Validate(); !Match(err, nil) {
		t.Errorf("%T: %s", err, err)
	}

	fh.FileCreationDate = time.Now().Format(ISO8601Format)
	if err := fh.Validate(); !Match(err, nil) {
		t.Errorf("%T: %s", err, err)
	}
...
func testFileHeaderCreationTime(t testing.TB) {
	...
	fh.FileCreationTime = time.Now().Format(ISO8601Format)
	if err := fh.Validate(); !Match(err, nil) {
		t.Errorf("%T: %s", err, err)
	}
